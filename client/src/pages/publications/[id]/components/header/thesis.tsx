  ...
  return (
    <section>
      <div>
        ...
        <Title className="fr-mb-1v" as="h1" look="h5">{getLangFieldValue(locale)(data.title)}</Title>
        <Text className="fr-text-mention--grey" size="sm">
          <em>
            {author.fullName && intl.formatMessage({ id: "publications.header.thesis.by" })}
            <Link href={`/authors/${author.id}`}>{author.fullName}</Link>
            {', '}
            {hasDate
              ? intl.formatMessage({ id: "publications.header.thesis.date" }, { date: new Date(data.publicationDate).toLocaleDateString('FR-fr', { year: 'numeric', month: 'long', day: 'numeric' }) })
              : intl.formatMessage({ id: "publications.header.thesis.year" }, { year: data.year })
            }
            {!!directors.length && intl.formatMessage({ id: "publications.header.thesis.directed" })}
