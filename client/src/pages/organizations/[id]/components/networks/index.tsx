...
export function OrganizationNetworksBadges({ data, title, icon }: { data: any, title: string, icon: string }) {
...
export function OrganizationNetworks({ data, title, icon }: { data: any, title: string, icon: string }) {
  const { locale } = useDSFRConfig();
  const intl = useIntl();
  if (!data?.length) return null;
  return (
