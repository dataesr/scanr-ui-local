  ...
  return (
    <>
      <Modal
        id={id}
        size="lg"
        title={intl.formatMessage({ id: "search.top.filters.authors.title" })}
      >
        <Container fluid className="fr-my-2w">
          ...
          <TagGroup>
            {byAward
              .slice(0, seeMoreAwards ? 10000 : SEE_MORE_AFTER)
              .map((type) => (
                <SelectableTag
                  selected={currentFilters?.["awards.label"]?.values
                    ?.map((v) => v.value)
                    ?.includes(type.value)}
                  key={type.value}
                  onClick={() =>
          ...
          {seeMoreAwards ? (
            <Button
              variant="text"
              size="sm"
              onClick={() => setSeeMoreAwards(false)}
            >
            ...
            <Button
              variant="text"
              size="sm"
              onClick={() => setSeeMoreAwards(true)}
            >
              {intl.formatMessage({ id: "search.filters.see-more" })}
            </Button>
          )}
