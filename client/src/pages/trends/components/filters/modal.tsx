...
function TrendsFiltersPublicationsModal() {
  const intl = useIntl()
  const { clearFilters } = useUrl()
  const { isFetching } = useTrends()
  const id = "trends-options-filters-modal"

  return (
    <>
      <Modal id={id} size="lg" title={intl.formatMessage({ id: "trends.filters.modal.publications.title" })}>
        ...
        <div className="fr-modal__footer fr-px-0" style={{ display: "flex", width: "100%", alignItems: "center" }}>
          <div style={{ flexGrow: 1 }}>
            <Button variant="secondary" onClick={() => clearFilters()}>
              {intl.formatMessage({ id: "trends.filters.modal.clear" })}
            </Button>
          </div>
          <Button
            disabled={isFetching}
          ...
          >
            {intl.formatMessage({ id: "trends.filters.modal.display" })}
          </Button>
        </div>
      </Modal>
    </>
...
export default function TrendsFiltersModal() {
  return <TrendsFiltersPublicationsModal />
}
