...
export default function YearRange({ data }) {
  const slider = useRef(null);
  const lowThumb = useRef(null);
  const highThumb = useRef(null);
  const [sliderLowPosition, setSliderLowPosition] = useState(0);
  const [sliderHighPosition, setSliderHighPosition] = useState(0);
  ...
  const [lowYear, setLowYear] = useState(null);
  const [highYear, setHighYear] = useState(null);
}
